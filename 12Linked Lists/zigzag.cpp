...
void zigzag(Node* head){
    ...
    // half of the list is reversed and prev points at the last node
    slow = prev;
    fast = head;

    // now we'll do alternate merging of the list
    while(fast&&slow){
        Node* fNext = fast->next, *sNext = slow->next;
        fast->next = slow;
        slow->next = fNext;
        fast = fNext;
        slow = sNext;
